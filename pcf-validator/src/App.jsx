...
function runValidation(dataTable, config, log) {
  ...
  for (let i = 0; i < dataTable.length; i++) {
    ...
    if (row.bore !== undefined && row.bore !== null && row.type !== "SUPPORT" && row.type !== "ISOGEN-FILES" && !row.type.startsWith("UNITS")) {
        ...
        // When reading from raw JSON, exact trailing zeros can be dropped. We pad valid ints to enforce formatting.
        if (strBore.match(/^\d+\.?\d*$/) && Number.isInteger(Number(strBore)) && strBore.includes('.')) {
            strBore = Number(strBore).toFixed(decimals);
        }
